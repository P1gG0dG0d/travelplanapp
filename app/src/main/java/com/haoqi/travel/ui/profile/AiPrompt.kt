...
fun buildPlanBrief(spec: PlanSpec, profile: ProfileEntity?, withTickets: Boolean = false): String {
    ...
    val rule7 = (if (withTickets) """
        7. 城际交通（去程、城际换乘、返程），按下面的要求逐项自查：
           a. **先按距离选交通方式**：长途（高铁约 4 小时以上，或约 800 公里以上）必须优先查航班、推荐飞机；中短途推荐高铁/火车；短途或没有机场的地方才推荐大巴。
           b. **线路全覆盖，一条都不能少**：去程（出发地→第1天城市）、每一段城际换乘、返程（最后一天城市→出发地）都要给出，尤其不要漏掉返程。
           c. **每条线路只出现一次**：查到真实班次→写进「# 车票」；没查到→写进「# 交通建议」；**绝不要同一条线路两边都给**。
           d. 联网搜索（web_search）：每条线路先搜一次，没查到可换关键词补搜一次，总共不超过 6 次；优先保证去程和返程。查完立即停止搜索，一次性输出完整行程 Markdown。搜索关键词参考：高铁/火车用「城市A到城市B 高铁 车次 日期」；机票用「出发城市 到达城市 航班 日期」（如「北京到广州 航班 2026-05-01」）。机票若两次都搜不到可靠的航班号和起降时刻，就把这条线路写成「方式: 飞机」的交通建议（注明大约飞行时长），**不要硬编航班号**。
    ...
    """ else """
        7. 城际交通单独列为「# 交通建议」：包含去程（出发地→第1天城市）、城际换乘、返程（最后一天城市→出发地），一条都不能少，尤其不要漏掉返程。
           **先按距离选交通方式**：长途（高铁约 4 小时以上，或约 800 公里以上）必须优先推荐飞机；中短途推荐高铁/火车；短途或没有机场的地方才推荐大巴。
