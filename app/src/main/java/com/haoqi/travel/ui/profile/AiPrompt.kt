...
fun buildPlanBrief(spec: PlanSpec, profile: ProfileEntity?, withTickets: Boolean = false): String {
    ...
    val rule7 = (if (withTickets) """
        7. 城际交通（去程、城际换乘、返程），按下面的要求逐项自查：
           ...
           d. 联网搜索（web_search）查询规则（务必照做，这是“一次就查到”的关键）：
              - **每条线路第一次搜索就必须用「带完整日期的精确关键词」**，例如 `北京 到 广州 航班 2026-05-01`、`广州 到 北京 高铁 车次 2026-05-03`；严禁只搜“北京到广州”这种没有日期的泛词。
              - 第一次没查到结果，**必须立刻换一种说法再搜一次**（如把“航班”换成“机票”、或加/换机场名）再下结论，不要第一次没结果就放弃。
              - 每条线路最多搜 2 次、总次数不超过 6 次，查到就停；查完立即停止搜索，一次性输出完整行程 Markdown。
              - 机票两次都没搜到可靠的航班号和起降时刻，才写成「方式: 飞机」的交通建议（注明大约飞行时长），**不要硬编航班号**。
